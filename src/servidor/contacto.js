...
class Contacto{
    ...
    /*
     * Recibe un correo nombre y mensaje si no existe en la base de datos lo inserta
     */
    agregarUsuario(nombre, correo, mensaje){
                    ...
                    con.query(`SELECT COUNT(*) AS usuario FROM USUARIOS WHERE correo = '${correo}'`,function(error, res, campo){
                        if(error){
                            ...
                            if(parseInt(res[0].usuario) == 0){
                                con.query(`INSERT INTO usuarios (nombre, correo, mensaje) values('${nombre}','${correo}','${mensaje}')`
